    ...
    <div className="flex flex-col justify-between h-full">
      <main className="p-5">
        ...
        <div className="flex flex-col justify-center items-center flex-1 space-y-10 mt-20 divide-y-4 divide-purple-400">
          {isLoading && <h1 className="text-lg text-purple-400 font-bold animate-pulse">Loading your NFT&apos;s, please wait...</h1>}
          {!isLoading && !wallet.connected && <h1 className="text-lg text-purple-400 font-bold">Please connect your wallet.</h1>}
          {!isLoading && wallet.connected && nfts.length === 0 && <h1 className="text-lg text-purple-400 font-bold">
            You do not have &quot;NFTs&quot;
          </h1>}

          {nfts.map((nft: any) => <div key={nft.name} className="flex pt-10 flex-col items-center 
            justify-center text-white font-bold uppercase space-y-5">
            ...
            <div className="flex flex-col text-xl space-x-3 items-center">
              ...
              <a target="_blank" href={`${nft.external_url}/${nft.id}`}
                className="flex space-x-3 text-purple-400 hover:text-purple-500" rel="noreferrer">
                <span>{nft.name}</span>
                <FontAwesomeIcon icon={faExternalLinkAlt} className="w-4" />
              </a>
              <span
                className="flex text-purple-400 space-x-2 text-sm">
